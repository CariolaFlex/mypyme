...
const FROM = process.env.RESEND_FROM ?? 'mypyme <no-reply@mypyme.cl>';
...
export async function enviarBienvenidaSuscripcion(p: {
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from: FROM,
      to: p.to,
      subject: '¡Tu suscripción a mypyme está activa! 🎉',
      html: `
        <div style="font-family:system-ui,-apple-system,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#1a1a1a">
          <div style="width:48px;height:48px;border-radius:14px;background:#5b4ddb;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:20px">m</div>
          <h1 style="font-size:20px;margin:20px 0 8px">¡Bienvenido a mypyme!</h1>
          ...
          <a href="https://mypyme-blond.vercel.app" style="display:inline-block;background:#5b4ddb;color:#fff;text-decoration:none;padding:10px 18px;border-radius:10px;font-weight:600">
            Ir a mi negocio
          </a>
